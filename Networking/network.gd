...
# Connect functions to the event signals
func _ready():
...
# this is the dictionary for all the players that are in the same server
var players = {}

# Events that are sent by Godot's networking system that we want to listen for
...
# initialize the network when create button is clicked in the create server panel
# this object will get its data from the widgits onClick
var server_info = {
	name = "Server",
	max_players = 0,
	used_port = 0
...
# function that maintains the current list of players on the server
remote func register_player(pinfo):
	if(get_tree().is_network_server()):
		...
		for id in players:
			...
			# Send new player info to currently iterated player, skipping
			# the server (which will get the info shortly)
			if(id != 1):
...
# remove player from LocalList when they leave server
...
# function to join a current server
...
# function to create Server (would be a good idea to validate info passed in)
